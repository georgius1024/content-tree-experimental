<template>
  <div
    class="flex items-center justify-between px-2 py-1 text-sm rounded transition hover:bg-gray-50 group"
  >
    <span class="inline-flex items-center gap-2 flex-1 min-w-0">
      ...
      <!-- Type icon -->
      <FileText
        v-if="step.type === 'slide'"
        :size="16"
        class="text-gray-500 shrink-0"
        aria-hidden="true"
      />
      <HelpCircle v-else :size="16" class="text-gray-500 shrink-0" aria-hidden="true" />

      <!-- Step name -->
      <span class="truncate">{{ step.name || t('stepList.unnamedStep') }}</span>
    </span>
...
import { FileText, HelpCircle, GripVertical, Pencil, Trash2 } from 'lucide-vue-next';
